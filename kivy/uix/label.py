...
"""
Label module with automatic Skia engine redirection
"""
import os

# Check graphics engine BEFORE any imports
_USE_SKIA = os.environ.get("GRAPHICS_ENGINE", "default") == "skia"

if _USE_SKIA:
    # Redirect to Skia implementation
    try:
        from kivy.uix.skia_label import SkiaLabel as Label
        from kivy.uix.skia_label import escape_markup

        __all__ = ("Label", "escape_markup")

    except ImportError as e:
        import warnings

        warnings.warn(
            f"Failed to import Skia Label: {e}. Falling back to default.",
            RuntimeWarning,
        )
        _USE_SKIA = False
...
if not _USE_SKIA:
    ...
    class Label(Widget):
        ...
        def __init__(self, **kwargs):
            self._trigger_texture = Clock.create_trigger(
        ...
        font_script_name = OptionProperty(
            "Latn",
            options=[
                "Zyyy",
                "Latn",
        ...
        font_direction = OptionProperty(
            "ltr", options=["rtl", "ltr", "ttb", "btt"]
        )
