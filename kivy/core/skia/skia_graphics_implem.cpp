...
private:
    ...
    void initialize() const {
        ...
        m_shape_path.setIsVolatile(true);
        m_initialized = true;
    }
    ...
    bool isFullCircle(float angleDiff) const { return std::abs(std::abs(angleDiff) - 360.0f) < FULL_CIRCLE_THRESHOLD; }

    bool isPerfectCircle(const EllipseGeometry& geom) const { return m_w == m_h && isFullCircle(geom.angleDiff); }
    ...
    void renderCircle(SkCanvas* canvas, const EllipseGeometry& geom) const {
        if (m_texture.hasTexture()) {
            m_shape_path.rewind();
            m_shape_path.addCircle(geom.centerX, geom.centerY, geom.radiusX);
            drawTexturedShape(canvas, m_shape_path, geom.bounds);
        } else {
    ...
    void renderEllipse(SkCanvas* canvas, const EllipseGeometry& geom) const {
        if (m_texture.hasTexture()) {
            m_shape_path.rewind();
            m_shape_path.addOval(geom.bounds);
            drawTexturedShape(canvas, m_shape_path, geom.bounds);
        } else {
    ...
    void renderArc(SkCanvas* canvas, const EllipseGeometry& geom) const {
        if (m_texture.hasTexture()) {
            m_shape_path.rewind();
            m_shape_path.addArc(geom.bounds, m_angle_start, geom.angleDiff);
            m_shape_path.lineTo(geom.centerX, geom.centerY);
            m_shape_path.close();
            drawTexturedShape(canvas, m_shape_path, geom.bounds);
        } else {
    ...
    void renderCustomSegments(SkCanvas* canvas, const EllipseGeometry& geom) const {
        ...
        pathBuilder.incReserve(m_segments + 3);
        ...
        pathBuilder.moveTo(geom.centerX, geom.centerY);

        for (int i = 0; i <= m_segments; ++i) {
            pathBuilder.lineTo(geom.centerX + geom.radiusX * cosAngle, geom.centerY + geom.radiusY * sinAngle);
        ...
        pathBuilder.close();
        m_shape_path = pathBuilder.detach();

        if (m_texture.hasTexture()) {
            drawTexturedShape(canvas, m_shape_path, geom.bounds);
        } else {
            canvas->drawPath(m_shape_path, m_paint);
        }
    }
